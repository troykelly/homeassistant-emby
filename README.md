...
## Using `media_player.play_media` & Assist voice search  _(updated for v0.7.0)_

Home Assistant exposes a generic `media_player.play_media` service which the
Emby integration fully implements.  It supports all parameters of the **2024.11
Core API** plus a backwards-compatibility layer for older releases.

| key        | required | type    | description                                                                                              |
|------------|----------|---------|----------------------------------------------------------------------------------------------------------|
| media_type | yes      | string  | `movie`, `episode`, `music`, `playlist`, `channel`, `trailer`                                             |
| media_id   | yes      | string  | Title, Emby ItemId, or an **`emby://` deep-link** generated by the browse dialog                          |
| enqueue    | no       | `string\|bool` | One of `"play"`, `"next"`, `"add"` (preferred) or legacy boolean (`true` → *next*, `false` → *play*) |
| announce   | no       | bool    | When `true` the item is played **without interrupting** existing playback where supported                |
...
data:
  ...
  enqueue: next            # or legacy `enqueue: true`
```
...
The same search engine powers Home Assistant’s **Assist** feature, allowing you
to trigger playback hands-free:

> “Hey Assistant, **play _Interstellar_ in the Living Room**”  
> “Hey Assistant, **queue the next episode of _The Office_ on the Projector**”  
> “Hey Assistant, **add _Random Access Memories_ to the Kitchen player**”

Assist translates the spoken command into a `media_player.play_media` service
call and the Emby integration resolves the requested title exactly as shown in
the YAML examples above.

Need to debug the matching?  Increase the log level for the component:
